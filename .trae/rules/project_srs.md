...
# 9. Acceptance Criteria (MVP/P1)

To keep this document concise, the full Acceptance Criteria for MVP/P1 have been moved to a dedicated file:
- .trae/rules/acceptance_criteria.md

This section covers FR1–FR8 for the MVP (fetch, list, search, details, favorites, offline persistence, navigation, and error handling).
# 10. Data Sources and Storage (MVP)

- Recipes (FR1–FR4)
  - Primary: External API via src/services/api.ts.
  - Fallback/seed: Local JSON in src/jsons/ (meals.json, meal_ingredients.json, ingredients.json) for demo/offline.
- Favorites (FR5–FR6)
  - Storage: AsyncStorage for persistence.
  - State: Centralized via context (e.g., src/context/RecipeContext.tsx) to expose add/remove/list operations.
- Navigation (FR7)
  - React Navigation setup in src/navigation/AppNavigator.tsx and App.tsx.
- Error Handling (FR8)
  - Global error state via src/context/ErrorContext.tsx.
  - UI surface via src/components/ErrorPopup.tsx.

# 11. Implementation Mapping & Checklist (P1)

- App & Navigation
  - [ ] Ensure App.tsx initializes providers (Theme, Recipe, Error) and mounts AppNavigator.
  - [ ] Configure stack/tabs for Home, Details, Favorites, Settings in src/navigation/AppNavigator.tsx.
- Fetch & List
  - [ ] Implement API calls in src/services/api.ts (list and search endpoints; include basic error handling).
  - [ ] Render recipe list in src/screens/HomeScreen.tsx using src/components/RecipeCard.tsx.
  - [ ] Show loading and empty states in HomeScreen.
- Search
  - [ ] Implement search flow in src/screens/SearchScreen.tsx (input, debounce, results, error/empty states).
- Details
  - [ ] Display image, ingredients with measures, and instructions in src/screens/RecipeDetailScreen.tsx.
  - [ ] Add Favorite toggle control in details.
- Favorites & Offline
  - [ ] Manage favorites in src/context/RecipeContext.tsx (add/remove/check, load/save via AsyncStorage).
  - [ ] Render saved list in src/screens/FavoritesScreen.tsx; support removal.
  - [ ] Verify favorites persist across restarts and work offline.
- Errors
  - [ ] Surface errors via src/components/ErrorPopup.tsx tied to src/context/ErrorContext.tsx.
  - [ ] Provide retry actions where applicable (Home/Search).
